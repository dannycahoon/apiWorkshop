...
RUN git clone ssh://git@orahub.oraclecorp.com/fmw-soa-dev/soa-apiplatform.git


# Capture a log of the git repository
WORKDIR /scratch/oracle/gitlocal/soa-apiplatform
RUN git log -n 1 --date=iso --format="Build from HEAD commit: %h on %ai" > /scratch/oracle/build.label
